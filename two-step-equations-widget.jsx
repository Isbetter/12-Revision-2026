...
function genQuestion(type) {
  ...
  const n = randInt(2, 9);
  const bMag = randInt(2, 18);
  let a = n;
  let b = bMag;
  let order = "variable-first";
  let tag = "add constant";

  if (type.startsWith("ax_minus")) {
    b = -bMag;
    tag = "subtract constant";
  } else if (type.startsWith("const_first")) {
    order = "constant-first";
    tag = "constant first";
  } else if (type.startsWith("neg_coeff")) {
    a = -n;
    tag = "negative coefficient";
  }
...
function equationString(q) {
  const absA = Math.abs(q.a);
  const varText = `${absA === 1 ? "" : absA}x`;
  const variable = `${q.a < 0 ? "-" : ""}${varText}`;
  const variableNoSign = varText;

  if (q.order === "constant-first") {
    return `${q.b} ${q.a < 0 ? "-" : "+"} ${variableNoSign} = ${q.c}`;
  }

  return `${variable} ${q.b < 0 ? "-" : "+"} ${Math.abs(q.b)} = ${q.c}`;
}
...
function QuestionExpression({ q }) {
  ...
  if (q.order === "constant-first") {
    return (
      <>
        <span className="teq-constant">{minusNumber(q.b)}</span>
        <span className="teq-sign">{q.a < 0 ? " \u2212 " : " + "}</span>
        <span className="teq-variable">{varText}</span>
      </>
    );
  }
  ...
  return (
    <>
      <span className="teq-variable">
        {q.a < 0 ? "\u2212" : ""}
        {varText}
      </span>
      <span className="teq-sign">{q.b < 0 ? " \u2212 " : " + "}</span>
      <span className="teq-constant">{Math.abs(q.b)}</span>
    </>
  );
}
...
export function TwoStepEquationsWidget() {
      ...
      <main className={`teq-card ${isRunning ? "is-running" : ""}`}>
        <header>
          <h1 className="teq-title">
            Equation Minute <span>{"\u00B7"} Two-step Equations</span>
          </h1>
          <p className="teq-subtitle">
            Timer optional {"\u00B7"} integer answers {"\u00B7"} keep moving
          </p>
        </header>
        ...
        {phase === "home" && (
          <section className="teq-home">
            ...
            <p className="teq-tagline">
              Leave the timer off for practice, or turn it on for a one-minute score. Positive and
              negative integer answers appear in balanced shuffled sets.
            </p>
          </section>
        )}
...
export default TwoStepEquationsWidget;
